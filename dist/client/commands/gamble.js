...
export default {
    ...
    execute: async (client, meta, message, args) => {
        ...
        if (win) {
            ...
            await client.chat.say(meta.channel, `ชนะ ${amount * 1.75} กีบ เหลือ ${balance.money + amount * 1.75} กีบ`);
            client.io.emit("feed", {
                type: "success",
                icon: "🎰",
                message: meta.user,
            ...
            await client.chat.say(meta.channel, `แพ้ ${amount} กีบ เหลือ ${balance.money - amount} กีบ`);
            client.io.emit("feed", {
                type: "danger",
                icon: "🎰",
                message: meta.user,
