...
export default {
    ...
    execute: async (client, meta, message, args) => {
        ...
        const songURL = song.match(/^((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube(-nocookie)?\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|live\/|v\/)?)([\w\-]+)(\S+)?$/)
            ? song
        ...
        await client.chat.say(meta.channel, `@${meta.user} เพิ่มเพลง "${songInfo.videoDetails.title}" โดย ${songInfo.videoDetails.author.name} เป็นคิวที่ ${songQueue.length}`);
    },
};
